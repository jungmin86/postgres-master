			 ...
			 RelOptInfo *baserel, ParamPathInfo *param_info)
{
	...
	// 디버그 로그에 출력
	// elog(DEBUG1, "Seq Scan on %s(%u): start_cost=%f, total_cost=%f, rows=%d width=%d",
	// 	get_rel_name(baserel->relid),
	// 	baserel->relid,  // get_relname_relid로 얻은 relid 사용
	// 	path->startup_cost, path->total_cost,
	// 	path->rows, path->pathtarget->width);
}
		   ...
		   bool partial_path)
{
	...
	// elog(DEBUG1, "Index Scan on %s: startup cost=%f, total cost=%f, rows=%.0f, width=%d",
    //  get_rel_name(path->indexinfo->rel->relid),
					  ...
					  JoinPathExtraData *extra)
{
	...
	// elog(DEBUG1, "Initial NestLoop join cost assessment: startup_cost=%f, total_cost=%f, run_cost=%f",
    //      startup_cost, startup_cost + run_cost, run_cost);
					...
					JoinPathExtraData *extra)
{
	...
	// elog(DEBUG1, "Final NestLoop join cost assessment: startup_cost=%f, total_cost=%f, run_cost=%f",
    //      startup_cost, startup_cost + run_cost, run_cost);
					   ...
					   JoinPathExtraData *extra)
{
	...
	// elog(DEBUG1, "Initial merge join cost calculation: startup_cost=%f, total_cost=%f",
    //      startup_cost, startup_cost + run_cost + inner_run_cost);
					 ...
					 JoinPathExtraData *extra)
{
	...
	// elog(DEBUG1, "Final merge join cost assessment: startup_cost=%f, total_cost=%f, run_cost=%f",
    //      startup_cost, startup_cost + run_cost, run_cost);
					  ...
					  bool parallel_hash)
{
	...
	// elog(DEBUG1, "Initial Cost Hash Join: startup_cost=%f, run_cost=%f, inner_path_rows=%f, outer_path_rows=%f, numbatches=%d",
	//      startup_cost, run_cost, inner_path_rows, outer_path_rows, numbatches);
					...
					JoinPathExtraData *extra)
{
	...
	// elog(DEBUG1, "Final Cost Hash Join: startup_cost=%f, total_cost=%f",
